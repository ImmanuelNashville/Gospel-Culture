...
export const getStaticProps = async () => {
  ...
  const courses = response.items.filter(
    (course) =>
      ...
      course.fields.category.find((cat) => cat.fields.name === 'Skills')
  );
...
export const SkillsPage = ({ courses, featuredCourses }: InferGetStaticPropsType<typeof getStaticProps>) => {
  const [primary, secondaryTop, secondaryBottom] = featuredCourses;

  return (
    <Layout title="Travel Skills" description="Bright Trip" fullBleed>
      <FullWidthSection bgColor="bg-gradient-to-tr from-bt-orange-darker to-bt-orange-light dark:to-bt-orange-light/30">
        <h1 className="text-3xl md:text-4xl font-bold text-white/90">Travel Skills Courses</h1>
        <p className="font-bodycopy text-md md:text-lg text-white/70 mb-4">
          Learn valuable skills that can help you take on any destination
        </p>
      </FullWidthSection>
      <SectionWithMargin>
        <SectionHeading title="Most Popular" description="Our community's favorite skills-based courses" />
      ...
      <SectionWithMargin>
        <SectionHeading title="Full Skills Library" description="All of our skills-based courses from our creators" />
